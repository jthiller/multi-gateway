    ...
    tokio::spawn(async move {
        ...
        loop {
            tokio::select! {
                ...
                _ = interval.tick() => {
                    let secs = watchdog_health.seconds_since_last_heartbeat();
                    if secs > 120 {
                        tracing::error!(
                            seconds_silent = secs,
                            "DISPATCHER STALL DETECTED: dispatch loop has not advanced for {}s",
                            secs
                        );
                    } else if secs > 60 {
                        tracing::warn!(
                            seconds_silent = secs,
                            "dispatch loop has not advanced for {}s",
                            secs
                        );
                    }
                }
