...
impl GatewayTable {
    ...
    /// happens **outside** the lock. Keeping the blocking send out of the lock
    /// stops a slow gRPC task from stalling the whole gateway table behind the
    /// write lock — API reads and other gateways' table access keep flowing,
    /// which breaks the lock-cycle deadlock with the semtech-udp runtime's
    /// internal bounded channels. The dispatcher still awaits this send inline,
    /// so a single gateway whose bounded channel is full can apply backpressure
    /// to the dispatch loop; the DispatcherHealth watchdog surfaces a loop that
